...
public class FAF extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getLogger().info("FAF " + (configFound ? "(Paper config found)" : "(fallback mode)")
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!command.getName().equalsIgnoreCase("faf")) return false;

        toggled = !toggled;
        applyState();
        getConfig().set("enabled", toggled);
        saveConfig();
        sender.sendMessage("\u00a76[FAF] \u00a7fVanilla attribute swap: "
                + (toggled ? "\u00a7aON" : "\u00a7cOFF"));
        return true;
    }
